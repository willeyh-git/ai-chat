    ...
    <div class="flex-1">
      <ul class="divide-y divide-gray-100 dark:divide-gray-700">
        <li
          ...
          :class="{
            'bg-blue-50 dark:bg-blue-900/30 border-r-4 border-r-blue-500': selectedSessionId === s.id,
          }"
        ...
        >
          <div class="p-4">
            <div class="flex items-center gap-3">
              ...
              <span class="text-xs text-gray-400 whitespace-nowrap">{{ new Date(s.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }}</span>
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{{ s.title || "Untitled" }}</p>
              </div>
              <!-- Message Count -->
              <span class="text-xs text-gray-400 whitespace-nowrap">{{ s.messages.length }}</span>
            </div>

            <!-- Session Actions (inline buttons) -->
            <div class="flex items-center gap-1 mt-3 pt-2 border-t border-gray-100 dark:border-gray-800">
              <button
              ...
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 9h2a2 2 0 002-2V7a2 2 0 01-2-2H6a2 2 0 00-2 2v2a2 2 0 012 2zm5-9l3.857-2.485c.905-.567 1.745.512 1.745 1.5v6c0 .986-1.189 1.05-1.5 1.43L14.125 17H14a2 2 0 00-2-2h-3z"/>
                </svg>
              </button>
              <button
...
async function handleNewChat() {
  ...
  emit("sessionSelect", undefined as any); // Will be handled by parent
}

async function handleActions(sessionId: number | null, action: 'edit' | 'duplicate' | 'delete', currentTitle?: string) {
  ...
  switch (action) {
    ...
    case 'duplicate':
      try {
        const id = await emitStore.duplicateSession(sessionId);
        onMenuToggle();
      } catch (e) {
