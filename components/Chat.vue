...
<ul class="space-y-2 mb-4">
  <li v-for="s in sessions" :key="s.id" @click="selectedSessionId = s.id"
      :class="{'bg-blue-100': selectedSessionId === s.id}" class="border-b pb-2 cursor-pointer p-1 rounded">
    {{ new Date(s.createdAt).toLocaleTimeString() }}
  </li>
</ul>
<div v-if="selectedSessionId !== null">
  <ul class="space-y-2 mb-4">
    <li v-for="(m, idx) in currentMessages" :key="idx" class="p-2 rounded bg-gray-100 dark:bg-gray-700">
  ...
  <button @click="send" :disabled="loading || !newMessage"
